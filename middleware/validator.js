...
exports.validateevent = [
    ...
    body('starttime', 'Start time cannot be empty').notEmpty().matches(/^(?:\d{4}-\d{2}-\d{2})T(?:[01]\d|2[0-3]):(?:[0-5]\d)$/i).withMessage('Invalid time format').trim().escape().custom((value, { req }) => {
    ...
    body('endtime', 'End time cannot be empty').notEmpty().matches(/^(?:\d{4}-\d{2}-\d{2})T(?:[01]\d|2[0-3]):(?:[0-5]\d)$/i).withMessage('Invalid time format').trim().escape(),
    body()
        .custom((value, { req }) => {
